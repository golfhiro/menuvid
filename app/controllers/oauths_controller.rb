class OauthsController < ApplicationController
  ...
  def callback
    ...
    if auth_params[:error].present?
      redirect_to profile_path, alert: 'ログイン(連携)をキャンセルしました'
      return
    end

    if @user = login_from(provider)
      redirect_to menus_path(date: Date.today), notice:(t'.login_success', item: provider.upcase)
    else
      begin
        ...
        redirect_to login_path, alert: (t'.login_failed', item: provider.upcase)
      end
    end
  end
